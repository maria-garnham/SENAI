...
export default function Lista_03() {
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.titulo}>Lista 03 - Maria Julia Garnham</Text>

      <View style={styles.card}>
        <Text style={styles.label}>Exercício 1 — Lista de Frutas</Text>
        {frutas.map((fruta, index) => (
          <Text key={index} style={styles.texto}>{fruta}</Text>
        ))}
      </View>
      ...
      <View style={styles.card}>
        ...
        {numeros.filter((numero) => numero % 2 === 0).map((numeroPar) => (
          <Text key={numeroPar} style={styles.texto}>{numeroPar}</Text>
        ))}
