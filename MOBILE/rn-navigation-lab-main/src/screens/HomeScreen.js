  ...
  function renderItem({ item }) {
    return (
      <TouchableOpacity
        ...
        onPress={() => navigation.navigate('Detalhe', { ...jogos })}
      >
        <View style={styles.cardIcone}>
          {/* TODO: substituir pela inicial do titulo ou outro elemento do seu tema */}
          <Text style={styles.cardIconeTexto}>{jogos.titulo[0]}</Text>
        </View>
        <View style={styles.cardInfo}>
          {/* TODO: substituir pelos campos do seu tema */}
          <Text style={styles.cardTitulo}>{jogos.titulo}</Text>
          <Text style={styles.cardSubtitulo}>{jogos.genero}</Text>
        </View>
      </TouchableOpacity>
    );
  }
